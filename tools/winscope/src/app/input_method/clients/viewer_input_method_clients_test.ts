...
import {ViewerInputMethodClients} from './viewer_input_method_clients';

describe('ViewerInputMethodClients', () => {
  beforeAll(() => {
  ...
  let viewer: ViewerInputMethodClients;
  ...
  beforeEach(async () => {
    ...
    viewer = new ViewerInputMethodClients(
      trace,
      new Traces(),
      new InMemoryStorage(),
    );
  ...
  it('adds component output listener for onAdditionalPropertySelected', async () => {
    const spy = spyOn(
      PresenterInputMethodClients.prototype,
