...
import {makeSpyRowIterator, setupMockIteratorWithRows,} from '@trace_processor/testing/test_utils';
...
describe('WindowManager RectExtractor', () => {
  it('handles row without rect id', () => {
    ...
    row.get.withArgs('rect_id').and.returnValue(null);

    const rect = extractRect(
      row,
      'WindowState 1234 WindowName',
      'WindowName',
      traceGeometryData,
    );
  ...
  it('extracts DisplayContent rect - not active', () => {
    checkExtractsDisplayRect(false);
  });

  it('extracts DisplayContent rect - active', () => {
    checkExtractsDisplayRect(true);
  });

  it('extracts WindowState rect', () => {
    const expectedRect = new TraceRectBuilder()
      .setX(1)
      .setY(2)
      .setWidth(3)
      .setHeight(4)
      .setId('WindowState 1234 WindowName')
      .setName('WindowName')
      .setGroupId(1)
      .setIsVisible(true)
      .setIsDisplay(false)
    ...
    row.get.withArgs('group_id').and.returnValue(1n);
    row.get.withArgs('depth').and.returnValue(2n);
    row.get.withArgs('opacity').and.returnValue(0);
    row.get
      .withArgs('container_type')
      .and.returnValue(ContainerType.WindowState);
  ...
  it('extracts all rects', () => {
    ...
    const expectedRects: RectsForTrace = new Map([
  ...
  function checkExtractsDisplayRect(isActive: boolean) {
    const expectedRect = new TraceRectBuilder()
      .setX(1)
      .setY(2)
      .setWidth(3)
      .setHeight(4)
      .setId('DisplayContent 1234 DisplayName')
    ...
    row.get
      .withArgs('container_type')
      .and.returnValue(ContainerType.DisplayContent);
    row.get.withArgs('focused_display_id').and.returnValue(isActive ? 1n : 0n);

    const rect = extractRect(
      row,
      'DisplayContent 1234 DisplayName',
      'DisplayName',
      traceGeometryData,
    );
    expect(rect).toEqual(expectedRect);
